...
```
 ...
 The AIC values are vary similar with the GAM model having a slightly higher AIC indicating that in this case it may be better to use a simple linear regression. 
 
```{r}
anova(linear_mod, gam1_lin, test="Chisq")
```
With a p-value of 0.1562, the anova test shows that the GAM model in this case had not improved the model significantly (p-value > 0.05).
